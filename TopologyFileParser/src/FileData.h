...
public:
  ...
  //! Return the current size
  GlobalIndexType size() const {return mElements->size();}

  //! Resize the array to contain size many elements
  void resize(GlobalIndexType size) {mElements->resize(size);}

  //! Resize the array to contain size many elements
  void resize(GlobalIndexType size, const DataClass& temp) {mElements->resize(size,temp);}
  ...
  DataClass& operator[](GlobalIndexType i) {return mElements->at(i);}

  //! Return the i'th attribute
  const DataClass& operator[](GlobalIndexType i) const {return mElements->at(i);}
...
template <class DataClass>
Data<DataClass>::Data(GlobalIndexType size) : mInternalArray(true)
{
