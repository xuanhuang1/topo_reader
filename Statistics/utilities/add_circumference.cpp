...
bool global_boundary_edge(uint32_t dim[], const float* x, const float* y)
{
  if (((fabs(dim[0] - x[0]) < 10e-6) && (fabs(dim[0] - y[0]) < 10e-6))
      || ((fabs(dim[1] - x[1]) < 10e-6) && (fabs(dim[1] - y[1]) < 10e-6))
      || ((fabs(dim[2] - x[2]) < 10e-6) && (fabs(dim[2] - y[2]) < 10e-6)))
    return true;
...
{
  if (argc < 11) {
    fprintf(stderr,"Usage: %s <family> <family_id> <simpl-id> <segmentation> <obj> <nr-of-coords> <dimx> dimy> <dimz> [binary | ascii] [restricted]\n", argv[0]);
    exit(0);
  }
  ...
  uint32_t bbox[3];
  bool ascii;
  bool restricted = false; // Should the computation be restructed to the interior
  ...
  if (strcmp(argv[10],"ascii") == 0)
    ascii = true;
  else if (strcmp(argv[10],"binary") == 0)
    ascii = false;
  else {
  ...
  if ((argc > 11) && (strcmp(argv[11],"restricted") == 0))
    restricted = true;
  ...
  for (it=hierarchy.allFeatures().begin();it!=hierarchy.allFeatures().end();it++) {
    ...
    if (feature_count % 100 == 0)
      fprintf(stderr,"Working on feature %d\n",feature_count);
    ...
    // Do a sanity check by resetting all vertex flags, inverting them
    // for all vertices part of the boundary and check whether all flags
    // are unset at the end (all vertices appear an even number of times
    // in the boundary
    for (i=0;i<vcount;i++)
      vflags[i] = false;
    ...
    float length = 0;
    // Now all edges that are marked should be the boundary of a feature
    for (mIt=edge_map.begin();mIt!=edge_map.end();mIt++) {
      if (eflags[mIt->second]) {
        //fprintf(stdout,"e: %d %d \n",mIt->first.mIndex[0],mIt->first.mIndex[1]);

        vflags[mIt->first[0]] = !vflags[mIt->first[0]];
        vflags[mIt->first[1]] = !vflags[mIt->first[1]];

        if (!restricted || !global_boundary_edge(bbox,&vertices[dimension*mIt->first[0]],&vertices[dimension*mIt->first[1]])) {
        length += dist(&vertices[dimension*mIt->first[0]],&vertices[dimension*mIt->first[1]]);
        }
      }
    }
    ...
    for (i=0;i<vcount;i++) {
      if (vflags[i]) {
        fprintf(stderr,"ERROR: Vertex %d only marked uneven number of times\n",i);
        assert (false);
      }
    }
    ...
    derived[feature_count++].mData = length;
    //break;
  }
  ...
  if (restricted)
    derived_handle.species("interior_circumference");
  else
    derived_handle.species("circumference");
